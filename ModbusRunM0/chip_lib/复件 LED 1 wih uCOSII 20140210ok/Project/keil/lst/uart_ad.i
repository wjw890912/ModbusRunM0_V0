...
void usart1_init(UART_T* UART)
{
	((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->P1_MFP = 0x00000400UL | 0x00000800UL;
	UART_ResetIP(((UART_T *) ((( uint32_t)0x40100000) + 0x50000)));
	((SYSCLK_T *) ((( uint32_t)0x50000000) + 0x00200))->APBCLK |= (1ul << 17); 
	((SYSCLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL1 = (((SYSCLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL1 &~0x03000000UL)|0x00000000UL;
  ((SYSCLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKDIV &=  ~(15<<8);
	UART->FCR  |= 6;
	UART->LCR |= (1ul << 3) |(1ul << 4) |(1ul << 5);
	((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->LCR &= ~(0x3ul << 0) ;
	((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->LCR |= (3) ;
	((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->LCR &= ~(1ul << 2);
	UART->FUNSEL = 0;
	UART1_Init();
	UART->IER |= (1ul << 0) |(1ul << 1) ;
	NVIC_EnableIRQ(UART1_IRQn);
	}
void UART1_IRQHandler(void)
{
	unsigned char a;
		 if(((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->ISR & (1ul << 8))                              
    {
	     ((a) = (((UART_T *) ((( uint32_t)0x40100000) + 0x50000)))->RBR); 
			
    }
	else if(((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->ISR & (1ul << 9))
	{
			...
			{
			sendlenn = 0;
			((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->IER	&= ~( (1ul << 1) );
			((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->LCR |= (1ul << 4) ;
			}
		}
}
...
void usart1_send(unsigned char dat1)
{
	  unsigned char k,a,n;
	       ...
	       for(k=0;k<8;k++)
				...
				((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->LCR &= ~(1ul << 4);
    	  ((((UART_T *) ((( uint32_t)0x40100000) + 0x50000)))->THR = (dat1)); 
